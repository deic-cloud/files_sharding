...
The desktop sync client and WebDAV clients belong on the user's **home** server. A client pointed at the master for a user homed on a silo is redirected there (`DAV/HomeRedirectPlugin`, 307 to the same path, method and body kept) — on `/remote.php/dav/files/<user>/…`, `/remote.php/webdav/…` and the legacy `/files/…`, `/grid/…`. It runs before authentication, since device passwords exist only on the home silo, and decides from the username alone (the path, or the Basic-auth header for the user-relative endpoints); the silo checks the credentials. Requests for another user's files and requests without a username are left alone. Note that clients generally do not resend credentials to a different host on redirect unless told to (curl: `--location-trusted`), so pointing a client straight at the home server remains the recommendation.
